...
def spherical_sample_histgram(I, L, camera_grid_positions, num_sampling_points, test_accurate_sampling, volume_position,
                              volume_size, c, deltaT, no_rho, start, end):
    [x0, y0, z0] = [camera_grid_positions[0], camera_grid_positions[1], camera_grid_positions[2]]

    if test_accurate_sampling:
        box_point = volume_box_point(volume_position,
                                     volume_size)  # return turn the coordinated of 8 points of the bounding box
...
def elliptic_sampling_histogram(I, L, camera_grid_positions, laser_grid_positions, num_sampling_points,
                                test_accurate_sampling, volume_position, volume_size, c, deltaT, no_rho, start, end,
                                device):
...
def show_samples(samples, volume_position, volume_size, camera_grid_positions):
    ...
    ax.scatter(samples[1:-1:showstep, 0], samples[1:-1:showstep, 1], samples[1:-1:showstep, 2], c='r', alpha=0.2,
               linewidths=0.01)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
...
def volume_box_point(volume_position, volume_size):
    ...
    x = np.array(
        [xv - volume_size / 2, xv - volume_size / 2, xv - volume_size / 2, xv - volume_size / 2, xv + volume_size / 2,
         xv + volume_size / 2, xv + volume_size / 2, xv + volume_size / 2])
    y = np.array(
        [yv - volume_size / 2, yv - volume_size / 2, yv + volume_size / 2, yv + volume_size / 2, yv - volume_size / 2,
         yv - volume_size / 2, yv + volume_size / 2, yv + volume_size / 2])
    z = np.array(
        [zv - volume_size / 2, zv + volume_size / 2, zv - volume_size / 2, zv + volume_size / 2, zv - volume_size / 2,
         zv + volume_size / 2, zv - volume_size / 2, zv + volume_size / 2])
...
def compute_loss(M, m, N, n, i, j, I, L, pmin, pmax, device, criterion, model, args, nlos_data, volume_position,
                 volume_size, c, deltaT, camera_grid_positions, s2, transform_matrix, transform_vector, *,
                 laser_grid_positions=0):
    [x0, y0, z0] = [camera_grid_positions[0, m * N + n + j], camera_grid_positions[1, m * N + n + j],
                    camera_grid_positions[2, m * N + n + j]]

    if args.test_neglect_former_bins:
        with torch.no_grad():
            if args.confocal:
                [input_points, I1, I2, I0, dtheta, dphi, theta_min, theta_max, phi_min,
                 phi_max] = spherical_sample_histgram(I, L, camera_grid_positions[:, m * N + n + j],
                                                      args.num_sampling_points, args.test_accurate_sampling,
                                                      volume_position, volume_size, c, deltaT, args.no_rho, args.start,
                                                      args.end)
            elif not args.confocal:
                [input_points, I1, I2, I0, dtheta, dphi] = elliptic_sampling_histogram(I, L, camera_grid_positions[:,
                                                                                             m * N + n + j],
                                                                                       laser_grid_positions[:,
                                                                                       m * N + n + j],
                                                                                       args.num_sampling_points,
                                                                                       args.test_accurate_sampling,
                                                                                       volume_position, volume_size, c,
                                                                                       deltaT, args.no_rho, args.start,
                                                                                       args.end, device)
        ...
        if args.refinement:
            network_res[:, 1] = network_res[:, 1] * (network_res[:, 1] >= 0.03) + (network_res[:, 1] < 0.03) * (
                        0.01 * network_res[:, 1])
            density = network_res[:, 1].reshape(I0, args.num_sampling_points ** 2)
        ...
        if args.hierarchical_sampling:
            with torch.no_grad():
                ...
                input_points_extra, samples_pdf = MCMC_sampling(network_res, theta_min, theta_max, phi_min, phi_max,
                                                                device, camera_grid_positions[:, m * N + n + j], c,
                                                                deltaT, args.no_rho, args.start, args.end, args)
    ...
    if args.save_fig:
        if ((n % 256 == 0) & (j == 0)) | ((m < 1) & (n < 32)):
            ...
            plt.title('grid position:' + str(format(x0, '.4f')) + ' ' + str(format(z0, '.4f')) + ' equal loss:' + str(
                format(loss_show, '.8f')) + ' coffe:' + str(format(loss_coffe.cpu().detach().numpy(), '.8f')))
            format(x0, '.3f')
    ...
    with torch.no_grad():
        ...
        if (not args.reflectance) & (not args.density):
            if not args.no_rho:
                ...
                for l in range(int(P * Q * R / test_batchsize)):
                    test_input_batch = test_input[0 + l * test_batchsize:test_batchsize + l * test_batchsize, :]
                    test_output[0 + l * test_batchsize:test_batchsize + l * test_batchsize] = model(
                        test_input_batch).view(-1)
            ...
            for l in range(int(P * Q * R / test_batchsize)):
                ...
                test_input_batch = encoding_batch_numpy(test_input_batch, args.encoding_dim, args.no_rho)
                test_input_batch = torch.from_numpy(test_input_batch).float().to(device)
                test_output[0 + l * test_batchsize:test_batchsize + l * test_batchsize, 0] = model(test_input_batch)[:,
                                                                                             0]
            test_volume = test_output[:, 1].view(P, Q, R)
            test_volume_rho = test_output[:, 0].view(P, Q, R)
            test_volume = test_volume.cpu().numpy()
...
def data_rebalance(args, total_loss, total_camera_grid_positions, nlos_data, camera_grid_positions, camera_grid_size,
                   index, device, *, total_laser_grid_positions):
...
def MCMC_sampling(network_res, theta_min, theta_max, phi_min, phi_max, device, camera_grid_position, c, deltaT, no_rho,
                  start, end, args):
    ...
    with torch.no_grad():
        ...
        result_list = []
        for k in range(L):
            result_list.append(pool.apply_async(func=MCMC_2, args=(
            pdf_map_numpy[k, :, :], args.num_MCMC_sampling_points ** 2, 0.3, theta_min, theta_max, phi_min, phi_max, k,
            args)))
            # if (k) % int(L / 10) == 0:
            #     print('MCMC:', k ,'/',str(L))
        pool.close()
